...
public class InteractibleManager : Singleton<InteractibleManager>
{
    ...
    void Update()
    {
        ...
        // if gazing at something(or nothing) different from previous frame
        if (FocusedGameObject != oldFocusedGameObject)
        {
            ...
            if (FocusedGameObject != null)
            {
                ...
                {
                    // 2.c: Send a GazeEntered message to be handled by the FocusedGameObject.
    ...
    // let a previously focused, interactable, gameobject know it is no longer in gaze focus
